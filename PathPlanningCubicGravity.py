...
L = 1 # side length of Cube
dt = 0.2*40 # integration time step
T = 30*80*5 # Total simulation time
scale = 100 # used to scale the co-ordinates for the swarm initialization
...
# Constants
grav_const = 3.986 * 10**14
rad_Earth = 6378.137 * 10**3
orbit_alt = 415 * 10**3 # approx. current ISS altitude in LEO: 415km
semiMajor = rad_Earth + orbit_alt # See notes for Choice of orbit and altitude/height


## Clohessy-Wiltshire equations

Origo = np.array([0,0,0]) # TODO Tweak Earth center location, maybe P,Dest too.
# TODO gravity_Pos and gravity_Dest are translated from Origo, but for NED you must ensure it is correctly rotated too right? not sure why NED required.
gravity_Pos = np.zeros(3) # to hold Pos w.r.t. Origo
gravity_Dest = np.zeros(3) # to hold Dest w.r.t. Origo
# TODO ask Dario to explain frames so know you have it right. Also slow everything down.
n= np.sqrt(grav_const/(semiMajor**3))
t_comp = T*0.05 # NOTE This often needs tweaking: should be total time in which final position should be reached from start. Possibly because global t variable is passed to the velocity gather function, which does not account for t_comp?
# Construct A and B matrices. These are re-computed with the updated time on-the-fly
A_mat = np.zeros((3,3))
B_mat = np.zeros((3,3))
B_inv = np.zeros((3,3))
def makeAMatrix(t_now):
    t = t_comp - t_now
    A_mat[0,0] = 4 - 3 * np.cos(n*t)
    A_mat[1,0] = 6 * (np.sin(n*t) - n*t)
    A_mat[1,1] = 1
    A_mat[2,2] = np.cos(n*t)
    return A_mat

def makeBinvMatrix(t_now):
    t = t_comp - t_now
    B_mat[0,0] = (1/n) * np.sin(n*t)
    B_mat[1,0] = (2/n) * (np.cos(n*t) - 1)
    B_mat[0,1] = (2/n) * (1 - np.cos(n*t))
    B_mat[1,1] = (1/n) * (4*np.sin(n*t) - 3*n*t)
    B_mat[2,2] = (1/n) * np.sin(n*t)
    B_inv = np.linalg.inv(B_mat)
    return B_inv

def gravity_Gather(P,Dest,t_step):
    t_now = t[t_step]# Current time used to construct A,B matrices in real time. There's just one global t value that's set before compute velocity stuff.
    A_mat = makeAMatrix(t_now)
    B_inv = makeBinvMatrix(t_now)
    gravity_Pos = P - Origo
    gravity_Dest = Dest - Origo
    return (1/N) * ( np.dot(B_inv,gravity_Dest) - np.dot(B_inv,np.dot(A_mat,gravity_Pos)))
    # NOTE np.dot(A,B) is matrix multiplication using numpy
...
def ComputeVelocity(P,t_step):
    ...
    for i in range(N):
        for j in range(N):
            # If ToDist for random co-ord is big, use gravitation, else use flat space v_g.
            if ( (np.absolute(P[0,0]-Dest[0,0])) > (5*L) ):
                v_G = gravity_Gather(P[i,:] , Dest[j,:], t_step)
            else:
                v_G = np.zeros(3)# NOTE set to 0 for debugging!
...
for i in range (steps):
    ...
    t_step = i # pass to gravity_gather; change name for clarity (many "i"s)
    Vel[:,:,i] = ComputeVelocity(Pos[:,:,i],t_step)
